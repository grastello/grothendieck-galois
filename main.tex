...
The Fundamental Theorem of Galois Theory is an old and profound (more than Eva, less than the Fundamental Theorem of Calculus) result of field theory.
...
In this pages we will explore Grothendieck's approach following closely \cite{edo}, but with some added details (mostly confined in the appendix).
...
The correspondance of Galois can then be retrieved by associating to each \(F\in\cat{C}\) the subgroup \(H\leq G\) such that the transitive \(G\)-set \(G/H\) is precisely the \(G\)-set associated to \(F\) by the equivalence above.

Theorem \ref{theo:galois} can also be used to give alternative proofs of other well-known results (such as the corespondence between subgroups of the fundamental group and path-connected covering spaces of a path-connected topological space) that intuitively share some similarities with the case of Galois Theory.
